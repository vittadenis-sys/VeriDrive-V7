...
const OWN_CAR = CUSTOMER_SERVICE_GROUPS.own_car;
const BUYING = CUSTOMER_SERVICE_GROUPS.buying_used;
...
export function BookingForm() {
  ...
  const customerServices = [...new Set([...OWN_CAR, ...BUYING])] as ServiceKey[];
  ...
  return <form action={submit} className="panel form">
    ...
    <div className="full"><label>Servizio<select name="service" value={service} onChange={(event) => handleServiceChange(event.target.value as ServiceKey)}>{customerServices.map((key) => <option key={key} value={key}>{VERIDRIVE_SERVICES[key].name} — €{VERIDRIVE_SERVICES[key].priceCents / 100}</option>)}</select></label></div>
    ...
    {!isOnline && <><label className="full">Dove si trova l'auto?<input name="location" required placeholder="Indirizzo, CAP o città" /></label><label>Data preferita<input name="date" type="date" required /></label><label>Orario preferito<input name="slot" required placeholder="Es. 14:00" /></label><label className="full" style={{ display: "flex", gap: 12, alignItems: "center" }}><input name="urgency" type="checkbox" checked={urgency} onChange={(event) => setUrgency(event.target.checked)} style={{ width: 22, height: 22 }} /><span><b>Urgenza +25 €</b><br /><small>Disponibilità tra 24 e 48 ore, quando presente.</small></span></label></>}
