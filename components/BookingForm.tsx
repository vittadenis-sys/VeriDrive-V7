...
export function BookingForm(){
  ...
  useEffect(()=>{
    ...
    (async()=>{
      try{
        ...
        if(!cancelled) setDemoAccess(data.customer?.demo_access===true);
      }catch{/* Booking can continue normally when profile lookup is unavailable. */}
    })();
  ...
  async function submit(form:FormData){
    ...
    try{
      ...
      const checkout=await checkoutResponse.json(); if(!checkoutResponse.ok){setMessage(checkout.error||"Prenotazione creata, ma impossibile completare il pagamento.");return;}
      if(checkout.demo){window.location.href=`/dashboard?booked=${encodeURIComponent(data.bookingId)}`;return;}
  ...
  return <form action={submit} className="panel form">
    ...
    {demoAccess&&<div className="full notice" style={{marginTop:0}}>Accesso Demo attivo: per questo account la prenotazione è gratuita e non viene richiesto il pagamento.</div>}
    ...
    <button className="button full" disabled={busy} type="submit">{busy?(demoAccess?"Conferma prenotazione…":"Apertura pagamento…"):(demoAccess?"Prenota gratuitamente":`Paga €${price.toFixed(2).replace('.',',')} e continua`)}</button>
