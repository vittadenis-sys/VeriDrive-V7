...
type Merchant = { id: string; company_name?: string | null; business_name?: string | null; full_name?: string | null; phone?: string | null; credits?: number | null; credit?: number | null };
...
export default function AdminCommerciantiPage() {
  ...
  async function load(nextSearch = search) {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/commercianti${query}`, { cache: "no-store" });
  ...
  return <>
    ...
    <main className="page"><div className="shell">
      ...
      <h1>Commercianti</h1>
      <p className="lead">Ricerca e gestione separata delle aziende e dei crediti.</p>
      <div className="panel admin-search" style={{ marginTop: 20 }}>
      ...
      <section className="panel admin-list-panel" style={{ marginTop: 20 }}>
        {loading ? <p>Caricamento commercianti…</p> : merchants.length === 0 ? <p style={{ marginBottom: 0 }}>Nessun commerciante trovato.</p> : <div className="admin-list">
          {merchants.map((merchant) => <article className="card admin-list-card" key={merchant.id}>
            <div>
              <strong>{merchant.company_name || merchant.business_name || merchant.full_name || "Commerciante"}</strong>
              {merchant.full_name && (merchant.company_name || merchant.business_name) && <div className="muted">{merchant.full_name}</div>}
              {merchant.phone && <div className="muted">{merchant.phone}</div>}
            </div>
            <div className="metric"><span>Crediti</span><strong>{merchant.credits ?? merchant.credit ?? 0}</strong></div>
          </article>)}
        </div>}
      </section>
    </div></main>
