  ...
  return <>
    ...
    <main className="page">
      <div className="shell" style={{ maxWidth: 900 }}>
        ...
        <h1 style={{ fontSize: "clamp(38px, 6vw, 60px)", marginBottom: 6 }}>Verifica pubblica</h1>
        {!certificate ? (
          <section className="panel customer-info" style={{ marginTop: 10 }}><XCircle size={30} /><div><h2>Certificato non trovato</h2><p>Il codice indicato non corrisponde a un certificato VeriScore pubblico.</p></div></section>
        ) : (
          <>
            <section className="panel customer-info" style={{ marginTop: 10, padding: 18, position: "relative", overflow: "hidden" }}>
              <div>
                ...
                <h2 style={{ marginBottom: 4 }}>{[certificate.vehicle_make, certificate.vehicle_model].filter(Boolean).join(" ") || "Veicolo"}</h2>
                <p style={{ marginBottom: 0 }}>{certificate.vehicle_year ?? "Anno non indicato"} · {certificate.workshop_name ?? "Officina VeriDrive"}</p>
              </div>
              {score && <div style={{ marginTop: 4, display: "flex", justifyContent: "center" }} aria-label={`VeriScore ${Math.round(Number(certificate.veriscore) || 0)} su 100`}>
            ...
            <section className="panel" style={{ marginTop: 8 }}><h3>Dettagli della certificazione</h3><p style={{ marginBottom: 6 }}><b>Data verifica:</b> {formatDate(certificate.issued_at)}</p><p style={{ marginBottom: 0 }}><b>Officina:</b> {certificate.workshop_name ?? "Officina VeriDrive"}</p></section>
            <section className="panel" style={{ marginTop: 8 }}><div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}><ShieldCheck size={24} /><p style={{ margin: 0 }}>La presente pagina verifica l'esistenza del certificato associato al codice indicato. I dati personali del proprietario non vengono pubblicati.</p></div></section>
          </>
        )}
        <div style={{ marginTop: 12 }}><Link href="/verifica" className="button secondary">Verifica un altro certificato</Link></div>
      </div>
    </main>
    <Footer />
  </>;
