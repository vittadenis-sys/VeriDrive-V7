  ...
  return <>
    ...
    <main className="page">
      <div className="shell" style={{ maxWidth: 900 }}>
        ...
        <h1 style={{ fontSize: "clamp(38px, 6vw, 60px)", marginBottom: 4 }}>Verifica pubblica</h1>
        {!certificate ? (
          <section className="panel customer-info" style={{ marginTop: 8 }}><XCircle size={30} /><div><h2>Certificato non trovato</h2><p>Il codice indicato non corrisponde a un certificato VeriScore pubblico.</p></div></section>
        ) : (
          <>
            <section className="panel customer-info" style={{ marginTop: 8, padding: 20, position: "relative", overflow: "hidden" }}>
              <div>
                <CheckCircle2 size={34} />
                <div className="eyebrow" style={{ marginTop: 6 }}>{isPlus ? "CERTIFICATO AUTENTICO · VERISCORE PLUS" : "CERTIFICATO AUTENTICO · VERISCORE"}</div>
                <h2 style={{ marginTop: 4, marginBottom: 4 }}>{[certificate.vehicle_make, certificate.vehicle_model].filter(Boolean).join(" ") || "Veicolo"}</h2>
                <p style={{ marginBottom: 0 }}>{certificate.vehicle_year ?? "Anno non indicato"} · {certificate.workshop_name ?? "Officina VeriDrive"}</p>
              </div>
              {score && <div style={{ marginTop: 8, display: "flex", justifyContent: "center" }} aria-label={`VeriScore ${Math.round(Number(certificate.veriscore) || 0)} su 100`}>
            ...
            <section className="cards" style={{ gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", marginTop: 8 }}><div className="metric"><span>Codice certificato</span><strong style={{ fontSize: 18 }}>{certificate.public_code}</strong></div><div className="metric"><span>Targa</span><strong>{certificate.vehicle_plate}</strong></div><div className="metric"><span>Telaio</span><strong style={{ fontSize: 16 }}>{certificate.vehicle_vin}</strong></div><div className="metric"><span>Km certificati</span><strong>{certificate.vehicle_mileage.toLocaleString("it-IT")}</strong></div></section>
            <section className="panel" style={{ marginTop: 8, padding: 20 }}><h3 style={{ marginTop: 0, marginBottom: 8 }}>Dettagli della certificazione</h3><p style={{ marginBottom: 4 }}><b>Data verifica:</b> {formatDate(certificate.issued_at)}</p><p style={{ marginBottom: 0 }}><b>Officina:</b> {certificate.workshop_name ?? "Officina VeriDrive"}</p></section>
            <section className="panel" style={{ marginTop: 8, padding: 20 }}><div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}><ShieldCheck size={24} /><p style={{ margin: 0 }}>La presente pagina verifica l'esistenza del certificato associato al codice indicato. I dati personali del proprietario non vengono pubblicati.</p></div></section>
          </>
        )}
        <div style={{ marginTop: 10 }}><Link href="/verifica" className="button secondary">Verifica un altro certificato</Link></div>
      </div>
    </main>
    <Footer />
  </>;
