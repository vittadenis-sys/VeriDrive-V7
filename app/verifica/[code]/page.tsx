          ...
          <>
            <section className="panel customer-info" style={{ marginTop: 12, padding: 24, position: "relative", overflow: "hidden" }}>
              <div>
                ...
                <h2 style={{ marginTop: 4, marginBottom: 6 }}>{[certificate.vehicle_make, certificate.vehicle_model].filter(Boolean).join(" ") || "Veicolo"}</h2>
                <p style={{ marginBottom: 0 }}>{certificate.vehicle_year ?? "Anno non indicato"} · {certificate.workshop_name ?? "Officina VeriDrive"}</p>
              </div>
              {score && <div style={{ marginTop: 12, display: "flex", justifyContent: "center" }} aria-label={`VeriScore ${Math.round(Number(certificate.veriscore) || 0)} su 100`}>
            ...
            <section className="cards" style={{ gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", marginTop: 10 }}><div className="metric"><span>Codice certificato</span><strong style={{ fontSize: 18 }}>{certificate.public_code}</strong></div><div className="metric"><span>Targa</span><strong>{certificate.vehicle_plate}</strong></div><div className="metric"><span>Telaio</span><strong style={{ fontSize: 16 }}>{certificate.vehicle_vin}</strong></div><div className="metric"><span>Km certificati</span><strong>{certificate.vehicle_mileage.toLocaleString("it-IT")}</strong></div></section>
            <section className="panel" style={{ marginTop: 10, paddingTop: 20, paddingBottom: 20 }}><h3 style={{ marginTop: 0, marginBottom: 12 }}>Dettagli della certificazione</h3><p style={{ marginBottom: 8 }}><b>Data verifica:</b> {formatDate(certificate.issued_at)}</p><p style={{ marginBottom: 0 }}><b>Officina:</b> {certificate.workshop_name ?? "Officina VeriDrive"}</p></section>
            <section className="panel" style={{ marginTop: 10, paddingTop: 18, paddingBottom: 18 }}><div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}><ShieldCheck size={24} /><p style={{ margin: 0 }}>La presente pagina verifica l'esistenza del certificato associato al codice indicato. I dati personali del proprietario non vengono pubblicati.</p></div></section>
          </>
        )}
        <div style={{ marginTop: 14 }}><Link href="/verifica" className="button secondary">Verifica un altro certificato</Link></div>
      </div>
    </main>
    <Footer />
  </>;
