...
type InspectionResponse = {
  ...
  booking?: { service?: string | null; plate?: string | null; vin?: string | null; vehicle_mileage?: number | null; vehicle_make?: string | null; vehicle_model?: string | null; vehicle_year?: number | null };
};
...
export default function ChecklistClient({ bookingId }: Props) {
  ...
  const canClose = canCloseChecklist && hasVehicleIdentity && (!isPlus || photos.length === 10);
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!response.ok) throw new Error((data as { error?: string }).error ?? "Impossibile caricare la pratica.");
        if (!active) return;
  ...
  return <main className="page"><div className="shell">
    ...
    {isPlus && <section className="panel" style={{ marginTop: 18 }}>
      <div className="eyebrow">VERISCORE PLUS · 10 FOTO</div><h3>Documentazione fotografica</h3>
      <p style={{ opacity: .75 }}>Carica esattamente 10 foto dell'auto: carrozzeria esterna e vano motore. Verranno associate al certificato Plus.</p>
      <input type="file" accept="image/*" multiple disabled={photoBusy || photos.length >= 10} onChange={(event) => void uploadPhotos(event.target.files)} />
      <div style={{ display: "grid", gridTemplateColumns: "repeat(5,minmax(0,1fr))", gap: 8, marginTop: 14 }}>
        {Array.from({ length: 10 }).map((_, index) => { const photo = photos[index]; return <div key={index} style={{ aspectRatio: "4 / 3", borderRadius: 8, overflow: "hidden", border: "1px solid #d8e2f2", background: "#f6f9ff", display: "flex", alignItems: "center", justifyContent: "center" }}>{photo?.preview_url ? <img src={photo.preview_url} alt={`Foto ${index + 1}`} style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <span style={{ fontSize: 12, opacity: .55 }}>{index + 1}</span>}</div>; })}
      </div>
      <p style={{ marginTop: 10, marginBottom: 0, fontWeight: 700 }}>{photos.length}/10 foto caricate.</p>{photoBusy && <p className="notice" style={{ marginTop: 10 }}>Caricamento foto…</p>}
    </section>}
