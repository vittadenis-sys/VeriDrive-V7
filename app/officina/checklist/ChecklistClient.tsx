...
import { calculateVeriscore, type RoadTestResult, scoreLabel } from "@/lib/veriscore";
...
export default function ChecklistClient({ bookingId }: Props) {
  ...
  const [roadTest, setRoadTest] = useState<RoadTestResult>("not_performed");
  ...
  const score = useMemo(() => calculateVeriscore(checklist.map((item) => values[item.id])), [values]);
  ...
  async function saveInspection(close = false) {
    ...
    try {
      const checklistResults = checklist.map((item) => ({ id: item.id, area: item.area, label: item.label, weight: item.weight, result: values[item.id] ?? null }));
      const response = await fetch("/api/workshop/inspection", {
        ...
        body: JSON.stringify({
          ...
          vehicle: isCertificateService ? {
            plate: vehicle.plate.trim().toUpperCase(), make: vehicle.make.trim(), model: vehicle.model.trim(),
            year: vehicle.year ? Number(vehicle.year) : null, vin: vehicle.vin.trim().toUpperCase(),
  ...
  return (
    <main className="page"><div className="shell">
      ...
      {isCertificateService && <section className="panel" style={{ marginTop: 18 }}>
        <div className="eyebrow">DATI VEICOLO · CERTIFICATO</div>
        <h3>Conferma i dati prima della chiusura</h3>
        <p style={{ opacity: .75 }}>Per VeriScore e VeriScorePlus l'officina deve confermare targa, VIN e chilometraggio prima di chiudere.</p>
        <div className="form" style={{ marginTop: 12 }}>
          ...
          <label className="full">Telaio / VIN<input value={vehicle.vin} onChange={(e) => { setVehicle(v => ({ ...v, vin: e.target.value.toUpperCase() })); setMessage(""); }} /></label>
          <label className="full">Chilometri<input value={vehicle.mileage} type="number" min="0" step="1" inputMode="numeric" onChange={(e) => { setVehicle(v => ({ ...v, mileage: e.target.value })); setMessage(""); }} /></label>
        </div>
        {!hasVehicleIdentity && <p className="notice" style={{ marginTop: 12 }}>Per chiudere servono targa, VIN e chilometraggio.</p>}
      </section>}
      ...
      <div className="panel" style={{ position: "sticky", top: 86, zIndex: 2, display: "flex", alignItems: "center", gap: 18, flexWrap: "wrap", marginTop: 18 }}>
        ...
        <div><p style={{ marginBottom: 6 }}><b>{score}/100 · {scoreLabel(score)}</b></p><p style={{ margin: 0 }}>{completed}/{checklist.length} controlli compilati</p>{!canCloseChecklist && <small style={{ display: "block", marginTop: 6, opacity: .7 }}>Completa tutti i controlli per chiudere la pratica.</small>}</div>
      </div>

      {loading && <p className="notice" style={{ marginTop: 18 }}>Caricamento della pratica…</p>}

      <div className="checklist" style={{ marginTop: 24 }}>
        {checklist.map((item) => <div className="check" key={item.id} style={{ display: "block" }}>
          <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "flex-end" }}>
              ...
              <button type="button" className={`button ${values[item.id] === "critical" ? "" : "secondary"}`} onClick={() => setResult(item.id, "critical")}>Grave</button>
            </div>
          </div>
        </div>)}
      </div>
      ...
      <section className="panel" style={{ marginTop: 24 }}>
        <div className="eyebrow">PROVA SU STRADA · FACOLTATIVA</div>
        <h3>Esito prova su strada</h3>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <button type="button" className={`button ${roadTest === "not_performed" ? "" : "secondary"}`} onClick={() => setRoadTest("not_performed")}>Non effettuata</button>
          <button type="button" className={`button ${roadTest === "passed" ? "" : "secondary"}`} onClick={() => setRoadTest("passed")}>Superata</button>
          <button type="button" className={`button ${roadTest === "issue" ? "" : "secondary"}`} onClick={() => setRoadTest("issue")}>Anomalia rilevata</button>
        </div>
        <p style={{ marginTop: 10, marginBottom: 0, opacity: .72 }}>Non effettuata = nessuna variazione. Superata = +5. Anomalia rilevata = −5.</p>
      </section>

      <section className="panel" style={{ marginTop: 24 }}><h3>Note finali</h3><textarea value={notes} onChange={(event) => { setNotes(event.target.value); setMessage(""); }} placeholder="Annotazioni del tecnico..." rows={5} style={{ width: "100%" }} /></section>
