...
import { CalendarDays, FileCheck2, LogIn, Plus, ShieldCheck, Wrench } from "lucide-react";
          ...
          {message && (
            <section className="panel customer-info" style={{ marginTop: 28 }}>
              <div>
                ...
                <h2>{message}</h2>
                <p>Accedi con il tuo account VeriDrive per visualizzare le pratiche reali.</p>
              </div>
              <Link className="button" href="/login"><LogIn size={18} /> Accedi</Link>
            </section>
          )}
          ...
          {!message && (
            <>
                      ...
                      return (
                        <article className="customer-check" key={booking.id}>
                          ...
                          <div className="customer-check-score">
                            ...
                            {booking.status === "completed" && <Link className="button secondary" href={`/report?booking=${booking.id}`}>Apri certificato</Link>}
                          </div>
                        </article>
                      );
                    })}
              ...
              <section className="cards" style={{ gridTemplateColumns: "repeat(auto-fit,minmax(240px,1fr))", padding: "10px 0 28px" }}>
                ...
                <Link className="card" href="/officina"><Wrench size={22} /><h3 style={{ marginTop: 10 }}>Area officina</h3><p>Accesso operativo disponibile agli account officina abilitati.</p></Link>
              </section>
            </>
          )}
        </div>
