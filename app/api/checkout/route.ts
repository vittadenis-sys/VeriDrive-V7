...
export async function POST(request: Request) {
  ...
  const { data: customer } = await supabase.from("customers").select("id,demo_access").eq("auth_id", user.id).single();
  ...
  if (!process.env.STRIPE_SECRET_KEY) return NextResponse.json({ error: "Pagamento non configurato" }, { status: 503 });
  ...
  const urgency = storedAmount === service.priceCents + 2500;
  if (storedAmount !== getCustomerPriceCents(service.key, urgency)) return NextResponse.json({ error: "Importo della prenotazione non valido." }, { status: 400 });
  ...
  const session = await stripe.checkout.sessions.create({
    mode: "payment",
    line_items: [{ price_data: { currency: "eur", product_data: { name: service.name, description: [label || "Verifica veicolo", reference, booking.location || ""].filter(Boolean).join(" · ") }, unit_amount: storedAmount }, quantity: 1 }],
  ...
  const { error: updateError } = await supabase.from("bookings").update({ stripe_checkout_session_id: session.id }).eq("id", booking.id).eq("customer_id", customer.id);
