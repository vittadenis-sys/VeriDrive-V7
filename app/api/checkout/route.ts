...
export async function POST(request: Request) {
  ...
  const { data: booking, error: bookingError } = await supabase
    .from("bookings")
    .select("id, plate, vehicle_make, vehicle_model, customer_price_cents, service_key, stripe_checkout_session_id")
    .eq("id", bookingId)
  ...
  const stripe = new Stripe(process.env.STRIPE_SECRET_KEY);
  const label = [booking.vehicle_make, booking.vehicle_model, booking.plate].filter(Boolean).join(" · ");
  const amount = booking.customer_price_cents ?? VERIFICATION_PRICE_CENTS;

  if (amount !== VERIFICATION_PRICE_CENTS) {
