...
export async function POST(request: Request) {
  try {
    ...
    if (!["requested", "assigned"].includes(booking.status)) return NextResponse.json({ error: "La pratica non è assegnabile nello stato attuale." }, { status: 409 });
    ...
    const { data: schedule } = await db.from("workshop_schedule").select("slot_time").eq("workshop_id", workshopId).eq("slot_time", slot).eq("active", true).limit(1);
    if (!schedule?.length) return NextResponse.json({ error: "Slot non disponibile per questa officina." }, { status: 409 });
