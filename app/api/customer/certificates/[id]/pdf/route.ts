...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const navy = rgb("#10233f"), blue = rgb("#2e5fbe"), mint = rgb("#55b89a"), text = rgb("#17233c"), muted = rgb("#66758a"), border = rgb("#d8e0ea"), pale = rgb("#f5f8fb");

    // PAGE 1 — certificate, optimized for phone reading
    ...
    pdf.addImage(qr, "PNG", W - 49, 173, 28, 28);
    ...
    // PAGE 2 — technical summary (Standard) or intro to Plus photo section
    pdf.addPage();
    ...
    for (const g of groups.slice(0, 5)) {
      ...
      pdf.setFont("helvetica", "bold"); pdf.text(`${g.ok}/${g.total}`, W - 49, y + 9);
      pdf.setTextColor(...main); pdf.text(`${g.pct}%`, W - 24, y + 9);
      y += 18;
    }
    pdf.setTextColor(...text); pdf.setFont("helvetica", "bold"); pdf.setFontSize(12); pdf.text("Esito della verifica", 16, y + 9);
    pdf.setTextColor(...muted); pdf.setFont("helvetica", "normal"); pdf.setFontSize(8.5); pdf.text(plus ? "Documentazione fotografica disponibile nelle pagine successive." : "Il certificato riassume il risultato dei 50 controlli tecnici eseguiti dall'officina.", 16, y + 18);
    ...
    if (plus) {
      ...
      for (let pageIndex = 0; pageIndex < photoPages; pageIndex++) {
        ...
        for (let j = 0; j < 2; j++) {
          ...
          if (photoList[idx]) pdf.text("Immagine disponibile", x + boxW / 2, py + boxH / 2, { align: "center" });
          else pdf.text("Foto non disponibile", x + boxW / 2, py + boxH / 2, { align: "center" });
        }
      }
    }
