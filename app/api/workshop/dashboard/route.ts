...
export async function GET() {
  try {
    ...
    const customerIds = (bookings ?? []).map((b) => b.customer_id).filter((id): id is string => Boolean(id));
    const vehicleIds = (bookings ?? []).map((b) => b.vehicle_id).filter((id): id is string => Boolean(id));
    const bookingIds = (bookings ?? []).map((b) => b.id).filter((id): id is string => Boolean(id));
    ...
    const [{ data: customers, error: customersError }, { data: vehicles, error: vehiclesError }, { data: certificates, error: certificatesError }] = await Promise.all([
      customerIds.length ? db.from("customers").select("id,full_name,phone").in("id", customerIds) : Promise.resolve({ data: [], error: null }),
      vehicleIds.length ? db.from("vehicles").select("*").in("id", vehicleIds) : Promise.resolve({ data: [], error: null }),
      bookingIds.length ? db.from("veriscore_certificates").select("id,booking_id,public_code").in("booking_id", bookingIds) : Promise.resolve({ data: [], error: null }),
    ]);
    ...
    if (certificatesError) return NextResponse.json({ error: certificatesError.message }, { status: 500 });
    ...
    return NextResponse.json({ workshop, bookings: enriched, isSuperAdmin: admin?.role === "super_admin" });
  } catch (error) {
