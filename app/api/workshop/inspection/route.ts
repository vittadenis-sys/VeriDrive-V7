...
export async function PUT(request: Request) {
  try {
    ...
    const { data: booking, error: bookingError } = await db
      .from("bookings")
      .select("id,customer_id,workshop_id,status,service,overall_notes,vehicle_id,plate,vehicle_make,vehicle_model,vehicle_year,vin,vehicle_mileage,public_reference,practice_code")
      .eq("id", bookingId)
      .maybeSingle();
    if (bookingError) return NextResponse.json({ error: bookingError.message }, { status: 500 });
    if (!booking || booking.workshop_id !== workshop.id) return NextResponse.json({ error: "Pratica non trovata." }, { status: 404 });
    ...
    if (!body.close || !certificateService) {
      const { error: saveError } = await db.from("bookings").update({ overall_notes: notesJson }).eq("id", bookingId).eq("workshop_id", workshop.id);
    ...
    const { error: saveVehicleError } = await db
      .from("bookings")
      .update({ overall_notes: notesJson, plate: nextPlate, vehicle_make: nextMake || null, vehicle_model: nextModel || null, vehicle_year: nextYear == null || nextYear === "" ? null : Number(nextYear), vin: nextVin, vehicle_mileage: nextMileage })
      .eq("id", bookingId)
      .eq("workshop_id", workshop.id);
    if (saveVehicleError) return NextResponse.json({ error: saveVehicleError.message }, { status: 400 });

    const { data: existingCertificate, error: certificateReadError } = await db
