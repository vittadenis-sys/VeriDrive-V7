...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json({ booking: { id: booking.id, workshop_id: booking.workshop_id, service: booking.service, vehicle_id: booking.vehicle_id }, inspection: { checklist: Array.isArray(stored.checklist) ? stored.checklist : [], notes: stored.checklist_notes ?? null, passed_checks: Number(stored.passed_checks ?? 0), veriscore: Number(stored.veriscore ?? 0), completed_at: stored.completed_at ?? null } });
  } catch (error) {
...
export async function PUT(request: Request) {
  try {
    ...
    // Keep the read contract backward-compatible with the live schema.
    ...
    if (!body.close) {
      const { error } = await db.from("bookings").update({
        overall_notes: JSON.stringify({ ...previousNotes, checklist: checklistResults, checklist_notes: String(body.notes ?? "").trim() || null, passed_checks: passedChecks, completed_checks: completedChecks, veriscore, completed_at: previousNotes.completed_at ?? null }),
    ...
    if (certificateService) {
      ...
      if (authUserError || !authUser.user?.email) return NextResponse.json({ error: "Email cliente non disponibile." }, { status: 500 });
    ...
    const { error: closeError } = await db.from("bookings").update({ overall_notes: notesJson, status: "completed" }).eq("id", bookingId).eq("workshop_id", workshop.id);
